...
export async function integrateWithLocalCodebase(options: IntegrateOptions) {
  ...
  if (!options.apiKey) {
    console.error(
      `[ERROR] - ${chalk.yellow('--apiKey')} is required, you can find it on ${chalk.cyan(
        'https://khulnasoft.com/account/settings'
      )}`
    );
    failed = true;
  }
  ...
  if (failed) {
    const errorMessage =
      'It looks like the integration failed, please fix the above errors and try again. Our full integration guide can be found here: https://www.khulnasoft.com/c/docs/integrating-khulnasoft-pages';
  ...
  if (!options.skipInstall) {
    ...
    console.info('installing the @khulnasoft.com/react sdk...', IS_YARN);
    await installPackage('@khulnasoft.com/react');
  }
  ...
  let khulnasoftContentUrl;
  if (options.content) {
    khulnasoftContentUrl = `https://khulnasoft.com/content/${options.content}?source=khulnasoft-cli`;
    // open(khulnasoftContentUrl);
  }
  ...
  console.info(`
    ...
    Next Steps:
      ...
      ${options.content ? `• Then, open: ${chalk.cyan(khulnasoftContentUrl)}` : ''}
    `)
    )}
    ...
    • Commit and deploy the code updates to your dev/staging site so your team members can test Khulnasoft.com.

    • Once you've deployed to a remote url, you'll want to edit the ${chalk.cyan(
      'previewUrl'
    )} property of your model here:
       ${chalk.cyan(`https://khulnasoft.com/models/${options.model}`)}
  `);
}
