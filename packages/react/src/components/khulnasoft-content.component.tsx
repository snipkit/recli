...
import { GetContentOptions } from '@khulnasoft.com/sdk'

export interface KhulnasoftContentProps<ContentType> {
...
> {
  ...
  subscribeToContent() {
    this.subscriptions.add(
      khulnasoft.queueGetContent(this.props.modelName, this.props.options).subscribe(
        matches => {
  ...
  onClick = (reactEvent: React.MouseEvent<HTMLElement>) => {
    ...
    if (khulnasoft.autoTrack) {
      khulnasoft.trackInteraction(content.id, content.variationId, this.clicked, event)
    }
    if (!this.clicked) {
      this.clicked = true
    }
  ...
  render() {
    ...
    return (
      ...
      <div
        ...
        khulnasoft-content-id={this.state.data && this.state.data.id}
        khulnasoft-model={this.props.modelName}
      >
