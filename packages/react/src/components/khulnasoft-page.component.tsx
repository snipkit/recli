...
import { KhulnasoftContent } from './khulnasoft-content.component'
import { KhulnasoftBlocks } from './khulnasoft-blocks.component'
import { Khulnasoft, GetContentOptions, khulnasoft, Subscription, BehaviorSubject } from '@khulnasoft.com/sdk'
...
const fetch = Khulnasoft.isBrowser ? window.fetch : require('node-fetch')
...
export interface KhulnasoftPageProps {
  modelName?: string
  name?: string
  data?: any
  entry?: string
...
export class KhulnasoftPage extends React.Component<KhulnasoftPageProps, KhulnasoftPageState> {
  ...
  constructor(props: KhulnasoftPageProps) {
    super(props)
  ...
  static renderInto(elementOrSelector: string | HTMLElement, props: KhulnasoftPageProps = {}) {
    const element =
    ...
    return ReactDOM.hydrate(<KhulnasoftPage {...props} />, element)
  }
  ...
  componentDidMount() {
    if (Khulnasoft.isIframe) {
      parent.postMessage({ type: 'khulnasoft.sdkInjected', data: { modelName: this.name } }, '*')
    }
  }
            ...
            >
              ...
              {content ? (
                <React.Fragment>
                  ...
                  <KhulnasoftBlocks fieldName="blocks" blocks={content.data.blocks} />
                </React.Fragment>
              ) : (
                <KhulnasoftContent
                  // TODO: pass entry in
                  ...
                  {(data, loading, fullData) => {
                    ...
                    return data ? (
                      <div
                        ...
                        data-khulnasoft-variation-id={fullData.variationId}
                      >
                        {this.getCss(data) && (
                          <style dangerouslySetInnerHTML={{ __html: this.getCss(data) }} />
                        )}
                        {<KhulnasoftBlocks fieldName="blocks" blocks={data.blocks} />}
  ...
  handleKhulnasoftRequest(propertyName: string, optionsString: string) {
    ...
    if (options) {
      ...
      this.subscriptions.add(
        khulnasoft.queueGetContent(options.model, options).subscribe(matches => {
          if (matches) {
            this.updateState(ctx => {
  ...
  onContentLoaded = (data: any) => {
    ...
    if (data && (data.httpRequests || data.khulnasoftData) && !this.props.noAsync) {
      ...
      for (const key in data.khulnasoftData) {
        const url = data.khulnasoftData[key]
        if (url && !this.data[key]) {
          this.handleKhulnasoftRequest(key, this.evalExpression(url))
        }
      }
    }
  }
